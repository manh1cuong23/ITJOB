...
export const getTableData = <T extends any[]>(pageNumber = 1, pageSize = 10, totalData: T) => {
  ...
  if (pageSize >= total) {
    ...
    tableData.pageNumber = 1; //直接取第一页
  } else {
    //pageSize小于总数据长度，数据多余1页
    const num = pageSize * (pageNumber - 1); //计算当前页（不含）之前的所有数据总条数
