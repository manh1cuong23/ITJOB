...
const InputBasic = (props: IProps) => {
  ...
  const handleKeyDownCode =
    (isCode: boolean, isName: boolean, isTime: boolean, regex?: RegExp) =>
  ...
  const handlePaste =
    (isName: boolean) => (e: React.ClipboardEvent<HTMLInputElement>) => {
  ...
  return (
    <MyFormItem
      ...
      {...(isSpan ? { labelCol: { span: 24 }, wrapperCol: { span: 24 } } : {})} // Chỉ thêm nếu isSpan tồn tại
      form={form}>
      <MyInput
